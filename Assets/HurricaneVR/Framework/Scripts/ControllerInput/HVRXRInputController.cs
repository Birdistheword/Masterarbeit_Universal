        ...
        protected override void CheckButtonState(HVRButtons button, ref HVRButtonState buttonState)
        {
            ...
            switch (button)
            {
                ...
                case HVRButtons.ThumbTouch:
                    Device.TryGetFeatureValue(CommonUsages.thumbTouch, out var temp);
                    ThumbTouch = temp > 0f;
                    SetButtonState(button, ref buttonState, ThumbTouch);
                    break;
                case HVRButtons.TriggerTouch:
                    Device.TryGetFeatureValue(CommonUsages.indexTouch, out var indexTouch);
                    TriggerTouch = indexTouch > 0f;
                    SetButtonState(button, ref buttonState, TriggerTouch);
                    break;
                case HVRButtons.TriggerNearTouch:
                    Device.TryGetFeatureValue(CommonUsages.indexTouch, out var indexNearTouch);
                    TriggerNearTouch = indexNearTouch > 0f;
                    SetButtonState(button, ref buttonState, TriggerNearTouch);
                    break;
                case HVRButtons.ThumbNearTouch:
                    Device.TryGetFeatureValue(CommonUsages.thumbTouch, out var thumbNear);
                    ThumbNearTouch = thumbNear > 0f;
