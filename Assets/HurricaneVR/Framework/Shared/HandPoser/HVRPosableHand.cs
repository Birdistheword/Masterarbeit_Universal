    ...
    public class HVRPosableHand : MonoBehaviour
    {
        ...
        private void AddFingerCapsules(HVRPosableFinger finger)
        {
            ...
            for (var i = 0; i < finger.Bones.Count; i++)
            {
                ...
                capsule.height = Vector3.Distance(next.position, bone.Transform.position);
                if (CapsuleAddRadius)
                {
                    capsule.height += CapsuleRadius;
                }
        ...
        public HVRHandPoseData Mirror(MirrorAxis handMirrorAxis, Transform transformOverride = null)
        {
            ...
            if (MirrorSettings)
            {
                thumbOverride = MirrorSettings.UseThumbSetting ? MirrorSettings.ThumbSetting : null;
                indexMirror = MirrorSettings.UseIndexSetting ? MirrorSettings.IndexSetting : null;
                middleMirror = MirrorSettings.UseMiddleSetting ? MirrorSettings.MiddleSetting : null;
                ringMirror = MirrorSettings.UseRingSetting ? MirrorSettings.RingSetting : null;
                pinkyMirror = MirrorSettings.UsePinkySetting ? MirrorSettings.PinkySetting : null;
            }
        ...
        private HVRPosableFingerData MirrorFinger(HVRPosableFinger finger, HVRJointMirrorSetting mirrorOverride, List<HVRJointMirrorSetting> settings)
        {
            ...
            for (var i = 0; i < finger.Bones.Count; i++)
            {
                ...
                if (mirrorOverride != null)
                {
                    mirror = mirrorOverride;
                }
                else if (settings != null && i < settings.Count)
                {
                    mirror = settings[i];
                }
