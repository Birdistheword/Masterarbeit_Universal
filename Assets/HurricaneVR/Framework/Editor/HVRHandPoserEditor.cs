...
namespace HurricaneVR.Editor
{
    ...
    public class HVRHandPoserEditor : UnityEditor.Editor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            SetupBoneButtons(LeftPosableHand, ref _selectedLeftBone);
            SetupBoneButtons(RightPosableHand, ref _selectedRightBone);
        ...
        private void DrawHandHandles(Transform handleTarget, ref bool drawRotation)
        {
            ...
            if (!drawRotation)
            {
                var newPosition = Handles.PositionHandle(handleTarget.position, Quaternion.identity);
                if (newPosition != handleTarget.position)
                {
                    Undo.RecordObject(handleTarget, "Hand Moved");
                    handleTarget.position = newPosition;
                }
            }
            else
            {
                var newRotation = Handles.RotationHandle(handleTarget.rotation, handleTarget.position);
        ...
        private static void SetupBoneButtons(HVRPosableHand hand, ref Transform selectedBone)
        {
            if (hand)
            {
